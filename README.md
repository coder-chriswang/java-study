...
看了部长发的PPT，就敏捷工程和管理实践上补充一些自己的经验和看法：
1、结对编程和契约式测试驱动开发实践的难度是相对大的。
这两点对于团队成员的均值能力是很高的，而且在我以往的经验中，团队中也是没有涉及这两部分。
编程开发部分采取的是组内人员配比，按高级、中级、初级三挡开发能力1:1:2的比例进行分配，测试人员一般为2-3人，1-2人按照测试用例进行手动测试（测试用例是需进行专门的评审会），1人进行性能测试（主要是对频繁调用的接口）。
TDD这部分，很抱歉，我本人没有这方面的实践经验。

2、迭代开发的周期一般为两周，以10个工作日进行计算。
考虑到终端侧的开发特点，云端在配合方面是可以用月进行迭代周期单位设定的。
后端的开发比前端的开发日期早，一般提前3-5天。

3、需求拆解（Story设定）方面，产品经理参与的比重是比较大的。
这个环节的主要目的是为了前期的需求澄清，明确当前版本的迭代需求，好为后续的高保真原型图、开发功能的拆分、测试用例的编写提供基础，为PL、PM指定明确的项目发布计划提供参考依据。
